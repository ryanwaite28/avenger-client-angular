...
export class UserMessagesFragmentComponent implements OnInit, OnDestroy {
  ...
  getMessagings() {
    ...
    this.modelClient.user.get_user_messagings(this.you!.id, undefined, true).subscribe({
      next: (response: any) => {
  ...
  getMessages() {
    ...
    this.modelClient.user.get_user_messages(
      this.you!.id,
      other_user.id,
      min_id
    ).subscribe({
  ...
  sendMessage() {
    ...
    this.loading = true;
    this.modelClient.user.send_user_message(
      this.you!.id,
      other_user.id,

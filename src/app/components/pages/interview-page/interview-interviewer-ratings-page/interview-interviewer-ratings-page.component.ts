...
@Component({
  selector: 'app-interview-interviewer-ratings-page',
  templateUrl: './interview-interviewer-ratings-page.component.html',
  styleUrls: ['./interview-interviewer-ratings-page.component.scss']
})
export class InterviewInterviewerRatingsPageComponent {
  ...
  handle_route_data(data: Data) {
    ...
    if (this.interview) {
      this.get_interview_interviewer_ratings();
    }
  }
  ...
  get_interview_interviewer_ratings() {
    const min_id: number | undefined = this.interview.interviewer_ratings?.at(-1)?.id;
    this.modelClient.interview.get_interview_interviewer_ratings(this.interview.id, min_id)
    .subscribe({
      next: (response) => {
        this.interview.interviewer_ratings?.push(...response.data!);
